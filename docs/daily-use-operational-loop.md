...
7. After deterministic validation passes, Runtime and QA inspect the artifact. A malformed QA response gets up to two format retries before any semantic work. Only an explicit `APPROVED` or `CHANGES REQUESTED` verdict may advance the control loop; format exhaustion ends in `needs-human` without consuming semantic repairs. A valid `CHANGES REQUESTED` verdict triggers up to three bounded semantic Builder repairs, each followed by a fresh QA recheck under the same format guard. Wiki Curator runs only after QA explicitly approves; semantic exhaustion ends in `needs-human`. The latest attempted repair, last deterministically valid artifact, and latest completed QA are tracked independently, so a later malformed repair cannot erase the last reviewable deliverable.
