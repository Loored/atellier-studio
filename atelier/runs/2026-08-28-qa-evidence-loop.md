...
- Real 3-day Mongo/Ollama run `6a91ef00b71d3ad93af71cbd` completed all seven agent steps and persisted a passing QA checklist after semantic repair 1. A later parent-run evidence inspection showed aggregate readiness `changes-required` because the final artifact evidence could not be reconstructed; this run is not an operational success. The earlier smoke runner incorrectly treated technical completion as success, motivating the trustworthy-runner follow-up.
