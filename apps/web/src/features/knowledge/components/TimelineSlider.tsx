...
type Props = {
  ...
  snapshotTimestamps?: ReadonlyArray<string>;
};

export function TimelineSlider({ timestamps, value, onChange, snapshotTimestamps }: Props) {
  const range = useMemo(() => {
  ...
  const snapshotPercents = (snapshotTimestamps ?? [])
    .map((iso) => Date.parse(iso))
    .filter((n) => !Number.isNaN(n) && n >= range.min && n <= range.max)
    .map((n) => ((n - range.min) / (range.max - range.min)) * 100);
  ...
  return (
    <div className="flex items-center gap-3 rounded-full border border-white/10 bg-black/30 px-3 py-1.5">
      ...
      <div className="relative h-3 w-44">
        {snapshotPercents.map((p, idx) => (
          <span
            key={`tick-${idx}`}
            className="absolute top-0 h-3 w-px bg-violet-300/60"
            style={{ left: `${p}%` }}
            aria-hidden
          />
        ))}
        <input
          ...
          className="absolute inset-x-0 top-1 h-1 w-full cursor-pointer appearance-none rounded-full bg-white/10 accent-violet-400"
          style={{
